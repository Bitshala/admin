...
pub fn read_from_db(path: &PathBuf) -> Result<Table, AppError> {
    ...
    let mut stmt = conn.prepare("SELECT name, group_id, ta, attendance, fa, fb, fc, fd, bonus_attempt, bonus_answer_quality, bonus_follow_up, exercise_submitted, exercise_test_passing, exercise_good_documentation, exercise_good_structure, total, mail, week FROM students")?;
