...
pub async fn discord_participant_oauth(query: web::Query<OAuthQuery>) -> Result<impl Responder> {
            ...
            if status.is_success() {
                match resp.json().await {
                    ...
                    Err(e) => {
                        ...
                        let redirect_url = "http://localhost:5173/unauthorized".to_string();
    ...
    let user_in_db = match match_discord_username(&db_path, &user.username) {
        ...
        Err(e) => {
            println!("User not found in pb_cohort database: {} - Error: {:?}", user.username, e);
            false
        }
    };
    ...
    let redirect_url = if has_pb_role && user_in_db {
        ...
        format!(
            "http://localhost:5173/instructions/1?auth=discord&token={}&email={}&username={}",
            encoded_token, encoded_email, encoded_username
        )
    } else {
        // Unauthorized page - either no PB role or not in database
        let reason = if !has_pb_role { "missing PB role" } else { "not in database" };
